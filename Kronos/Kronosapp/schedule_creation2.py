...
def schedule_creation2():
    ...
    # Variables de penalización para la restricción suave
    penalty_no_consecutive = pulp.LpVariable.dicts(
        "penalty_no_consecutive",
        ((subject, course_schedule) for subject in subjects for course_schedule in course_schedules),
        cat="Binary"
    )
    ...
    # Incluir penalización en la función objetivo
    penalty_weight = 10
    problem += (
        pulp.lpSum(assignment[subject, course_schedule] for subject in subjects for course_schedule in course_schedules) 
        - penalty_weight * pulp.lpSum(penalty_no_consecutive[subject, course_schedule] for subject in subjects for course_schedule in course_schedules)
    )
    ...
    # Restricción suave para clases consecutivas en horas impares y pares
    for subject in subjects:
        for course_schedule in course_schedules:
            if "Hour" in course_schedule:
                day, hour_str, course_str = course_schedule.split("_")
                hour_num = int(hour_str.replace("Hour", ""))
                if hour_num % 2 == 1:  # Hora impar
                    next_hour_schedule = f"{day}_Hour{hour_num + 1}_{course_str}"
                    if next_hour_schedule in course_schedules:
                        # Penalizar si no hay consecutividad entre las horas impares y pares en el mismo día
                        problem += (
                            assignment[subject, course_schedule] + assignment[subject, next_hour_schedule] 
                            - penalty_no_consecutive[subject, course_schedule] <= 1
                        )

    # soft constraints

    # 1. Si una materia es asignada en un horario impar el sigiente horario debe ser asignado a la misma materia.
